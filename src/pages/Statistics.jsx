...
const Statistics = ({ games, onClose }) => {
    ...
    <div className="p-6 max-w-7xl mx-auto">
      ...
      <div className="bg-gray-800/50 rounded-lg p-6 shadow-lg mb-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-6">
          ...
          <div className="text-center">
            <div className="text-3xl mb-2">🏆</div>
            <div className="text-2xl font-bold text-yellow-400">{statusStats["100-percent"]}</div>
            ...
            <div className="space-y-3">
              {topPlayedGames.map((game, index) => (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="text-white font-semibold truncate">{game.name}</div>
                    <div className="text-sm text-gray-400 mt-1">⏱️ {formatTime(game.playTime)}</div>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="space-y-3">
            {recentGames.map((game, index) => (
              <div
                ...
                className="flex items-center gap-3 bg-gray-700/50 rounded-lg p-3 hover:bg-gray-700 transition-colors"
              >
                <div className="text-2xl font-bold text-blue-400 w-8 text-center">#{index + 1}</div>
                <div className="flex-1 min-w-0">
                  <div className="text-white font-semibold truncate">{game.name}</div>
                  <div className="text-sm text-gray-400 mt-1">📅 {formatDate(game.dateAdded)}</div>
                </div>
              </div>
            ))}
          </div>
